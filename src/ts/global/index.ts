...
const global: IGlobal = {
  ...
  compare(item1: any, item2: any): boolean {
    ...
    } else if (items.every(item => item === undefined)) {
      return true;
  ...
  addSymbolsForCase(target: any, symbol: string, addSymb: string = "", other = false): string {
    if (global.isString(target)) {
      // для PascalCase, camelCase
      if (other) {
        let str = "";
        ...
        if (!symbol) {
          ...
          if (!target.split("").includes(symbol))
            global.setError(`
                            Symbol: "${symbol}" must be in string "${target}"}
                        `);

          target.split(symbol).map(item => {
            item[0] ? str += `${item[0].toUpperCase()}${item}`.replace(item[0], "") : false;
          })
        }
      ...
      // для kebab-case, snake_case
      let res = "";

      const arr = target.split("");
      const keySymbols = ["upper", "lower", "space"];

      if (!arr.includes(symbol) && !keySymbols.includes(symbol))
        global.setError(`Symbol: "${symbol}" must be in string "${target}" or must be equals: ${keySymbols.join(", ")}`);

      for (let i = 0; i < arr.length; ++i) {
        // для ключевых символов
        if (keySymbols.includes(symbol)) {
          switch (symbol) {
            case "upper":
              if (arr[i].toUpperCase() === arr[i])
                arr.indexOf(arr[i]) !== 0 ? arr[i] = addSymb + arr[i] : false;
              break;
            case "lower":
            ...
            case "space":
              if (" " === arr[i])
                arr.indexOf(arr[i]) !== 0 ? arr[i] = addSymb + arr[i].replace(" ", "") : false;
              break;
            default: return "";
          }
        } else {
          // для личных символов
          if (symbol === arr[i] && arr.indexOf(arr[i]) !== 0)
            arr[i] = addSymb + arr[i].replace(symbol, "");
        }
